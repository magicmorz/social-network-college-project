...
exports.getUserProfile = async (req, res) => {
  try {
    ...
    const isFollowing = targetUser.followers ? 
      targetUser.followers.some(follower => follower.user.toString() === currentUserId.toString()) : 
      false;
...
exports.followUser = async (req, res) => {
  try {
    ...
    const alreadyFollowing = currentUser.following ? 
      currentUser.following.some(follow => follow.user.toString() === targetUser._id.toString()) : 
      false;
    ...
    const followerCount = updatedTargetUser.followers ? updatedTargetUser.followers.length : 0;
...
exports.getUserInsights = async (req, res) => {
  try {
    ...
    const isOwnProfile = currentUserId.toString() === targetUser._id.toString();
    if (!isOwnProfile) {
      return res.status(403).json({ error: 'Access denied. You can only view your own insights.' });
    }
