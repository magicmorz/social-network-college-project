...
exports.createPost = async (req, res) => {
  try {
    ...
    const postData = {
      user: req.user._id,
      caption: caption || "",
      media: `uploads/user_${req.user._id}/posts/${req.file.filename}`, // Changed from 'image' to 'media'
      type: mediaType,
      place: placeRef,
      group: group || null,
    ...
    console.log("About to save post:", postData);
...
exports.addComment = async (req, res) => {
  try {
    ...
    const validGifUrl = gifUrl || "";
    
    if (!trimmedText && !validGifUrl) {
...
exports.uploadImage = upload.single("media"); // Changed from "image" to "media" for consistency